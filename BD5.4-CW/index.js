...
app.get("/seed_tracks", async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Tracks database seeded successfully" });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error seeding tracks", error: error.message });
  }
});
...
app.post("/users/update/:id", async (req, res) => {
  try {
    ...
    const response = await updateUserById(id, newUserData);
